...
export async function POST(req: NextRequest) {
  try {
    ...
    const imageUrls = await generateImageVariations(imageBuffer, { size, n })

    // Track this generation for analytics
    await trackGeneration(userId, "dall-e-variations", "")
    ...
    return NextResponse.json({ error: "Failed to generate image variations" }, { status: 500 })
  }
}
